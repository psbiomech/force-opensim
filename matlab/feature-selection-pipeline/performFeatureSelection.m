...
indfeatout = IndFeat(training.data,pcainfo.isACLR);
...
for n=1:niter
    fprintf('\n----> Iteration: %d\n',n);
    cvpart = cvpartition(pcainfo.isACLR,'k',10);    % create k new partitions on every iteration
    fs = sequentialfs(fun,weissind.data,pcainfo.isACLR,'cv',cvpart,'NFeatures',10,'Direction','forward','options',opts);
