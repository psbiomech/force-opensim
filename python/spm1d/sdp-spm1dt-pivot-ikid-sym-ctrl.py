...
# Required scenario
df = df1[df1["data_leg_role"] == "pivot"]
...
for a in analyses:
    ...
    for v in ["time"] + osimvars[a]:
        ...
        # More symptomatic
        symdata = df[(df["subj_type"] == "sym") & (df["trial_combo"] == "pivot_more") & (df["analysis"] == a) & (df["variable"] == v)]
        datamat[a][v]["sym"] = symdata.loc[:, "t1":"t101"].to_numpy()
        eventmat[a][v]["sym"] = symdata.loc[:, "es1_PFO1":"es6_PFS4"].to_numpy()
    
        # Controls
        ctrldata = df[(df["subj_type"] == "ctrl") & (df["trial_combo"].str.contains("pivot")) & (df["analysis"] == a) & (df["variable"] == v)]
        datamat[a][v]["ctrl"] = ctrldata.loc[:, "t1":"t101"].to_numpy()
        eventmat[a][v]["ctrl"] = ctrldata.loc[:, "es1_PFO1":"es6_PFS4"].to_numpy()
...
for gn, g in enumerate(subjtype):
    events["data"][g] = eventmat["ik"]["time"][g]
    events["desc"][g] = {}
