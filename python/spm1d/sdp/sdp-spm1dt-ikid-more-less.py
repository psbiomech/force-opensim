# -*- coding: utf-8 -*-
"""
STEP DOWN AND PIVOT: MORE SYM VS LESS SYM - SPM1D T-TEST (FOR PUBLICATION)
...
outfilename = "sdp-spm1dt-ikid-more-less"
...
subjtype = ["sym", "sym"]
subjtypefulllabel = ["more symptomatic", "less symptomatic"]

# Scenario labels
scenario = ["pivot", "nonpivot"]
trialcombo =[["pivot_more", "pivot_less"], ["pivot_less", "pivot_more"]]
...
for sn, n in enumerate(scenario):
    ...
    for a in analyses:
        ...
        for v in ["time"] + osimvars[a]:
            ...
            # Group 2
            gp2data = df[(df["subj_type"] == subjtype[1]) & (df["trial_combo"] == trialcombo[sn][1]) & (df["analysis"] == a) & (df["variable"] == v)]
...
for nv, n in enumerate(scenario):
    ...
    # save to pdf
    plt.savefig(os.path.join(outpath, outfilename + bonftext[bonferroni] + "-" + n + ".pdf"))