# -*- coding: utf-8 -*-
"""
Run SPMt 1-D analyses: FORCE step-down-pivot SO (FORCE vs controls)
...
outpkl = "force-sdp-spm1dt-so-sex.pkl"
...
for lg in leg:
    ...
    for gp in groups:
        ...
        for sx in sexstr:
            mgmat[lg][gp][sx] = {}
            for mg in muscgrps:
                mgsum = np.zeros(np.shape(datamat0[lg]["so"][mg[1][0]][gp][sx]))
                for m in mg[1]:
                    mgsum = np.add(mgsum, datamat0[lg]["so"][m][gp][sx])
                mgmat[lg][gp][sx][mg[0]] = mgsum
...
for lg in leg:
    ...
    for gp in groups:
        ...
        for mg in muscgrps:               
            ...
            for sxs in sexstr:
                desc[lg][gp][mg[0]][sxs] = {}      
                desc[lg][gp][mg[0]][sxs]["mean"] = np.mean(mgmat[lg][gp][sxs][mg[0]], axis = 0)
                desc[lg][gp][mg[0]][sxs]["sd"] = np.std(mgmat[lg][gp][sxs][mg[0]], axis = 0)
...
for lg in leg:
    ...
    for gp in groups:
        ...
        for mg in muscgrps:     
            ...
            spmtinf[lg][gp][mg[0]] = {}
            for pa in pairs:
                Y0 = mgmat[lg][gp][pairs[pa][0]][mg[0]]
                Y1 = mgmat[lg][gp][pairs[pa][1]][mg[0]]
                spmt[lg][gp][mg[0]][pa] = spm1d.stats.ttest2(Y0, Y1, equal_var=False)
                spmtinf[lg][gp][mg[0]][pa] = spmt[lg][gp][mg[0]][pa].inference(alpha=0.05, two_tailed=True, interp=True)
...
for gp in groups:
    ...
    nsubjs = [np.size(datamat0["ipsi"]["so"]["soleus"][gp][sxs], axis=0) for sxs in sexstr]
        ...
        for ln, lmb in enumerate(leg):
            ...
            for col in range(len(mgplots[0])):        
                ...
                for r, row in enumerate([0, 3]):        
                    ...
                    # mean
                    m0 = desc[lmb][gp][mgplots[r][col]][pairs[pa][0]]["mean"]
                    m1 = desc[lmb][gp][mgplots[r][col]][pairs[pa][1]]["mean"]
                    
                    # upper
                    u0 = m0 + desc[lmb][gp][mgplots[r][col]][pairs[pa][0]]["sd"]
                    u1 = m1 + desc[lmb][gp][mgplots[r][col]][pairs[pa][1]]["sd"]
                    
                    # lower
                    l0 = m0 - desc[lmb][gp][mgplots[r][col]][pairs[pa][0]]["sd"]
                    l1 = m1 - desc[lmb][gp][mgplots[r][col]][pairs[pa][1]]["sd"] 
                ...
                for r, row in enumerate([1, 4]):  
                    ...
                    for v in range(1, 5): ax.axvline(x=eventlist[v], linewidth=1.0, linestyle=":", color="k")
                    spmtinf[lmb][gp][mgplots[r][col]][pa].plot(plot_ylabel=False)
