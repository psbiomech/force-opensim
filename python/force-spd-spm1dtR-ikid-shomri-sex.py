...
outpkl = "force-sdp-spm1dtR-ikid-shomri-sex.pkl"
    ...
    for an in analyses:        
        ...
        for va in osimvars[an]:
            ...
            for gp in groups:       
                ...
                for sx in sex:
                    sxdata = gpdata[gpdata["sex"] == sx] 
                    sxdata = sxdata[["shomri_" + lg]]
                    shomri[lg][an][va][gp][sexstr[sx - 1]] = sxdata.to_numpy()
...
# run SPM{t} reression and inference
spmt = {}
spmtinf = {}
for lg in leg:
    spmt[lg] = {}
    ...
    for an in analyses: 
        ...
        for va in osimvars[an]:
            ...
            for gp in groups:
                ...
                spmtinf[lg][an][va][gp] = {}                
                for sxs in sexstr:
                    Y = datamat[lg][an][va][gp][sxs]
                    x = shomri[lg][an][va][gp][sxs].flatten()
                    spmt[lg][an][va][gp][sxs] = spm1d.stats.regress(Y, x)
                    spmtinf[lg][an][va][gp][sxs] = spmt[lg][an][va][gp][sxs].inference(alpha=0.05, two_tailed=True)
...
for g, gp in enumerate(groups):  
    ...
    for sxs in sexstr:   
        ...
        # number of subjects
        nsubjs = np.size(datamat["ipsi"]["ik"]["ankle_angle"][gp][sxs], axis=0)    
        
        for ln, lmb in enumerate(leg):
        
            # plot setup
            fig = plt.figure(constrained_layout=True, figsize=(50, 15))   
            fig.suptitle("Step-down-pivot - %s vs SHOMRI (n%d) - %s - %s limb" % (gp, nsubjs, sxs, limblabel[ln].title()), fontsize=20)
            heights = [2, 1, 0.5, 2, 1]
            ...
            for col in range(len(osimvars["ik"])):        
                ...
                for r, row in enumerate([0, 3]):        
                    ...
                    # mean
                    m0 = desc[lmb][an][osimvars[an][col]][gp][sxs]["mean"]
                    
                    # upper
                    u0 = m0 + desc[lmb][an][osimvars[an][col]][gp][sxs]["sd"]
                    
                    # lower
                    l0 = m0 - desc[lmb][an][osimvars[an][col]][gp][sxs]["sd"]    
                ...
                for r, row in enumerate([1, 4]):  
                    ...
                    for v in range(1, 5): ax.axvline(x=eventlist[v], linewidth=1.0, linestyle=":", color="k")
                    spmtinf[lmb][an][osimvars[an][col]][gp][sxs].plot(plot_ylabel=False)
                    
                    
            # save to pdf
            plt.savefig(os.path.join(outpath, outfigprefix + gp.lower() + "-" + sxs.lower() + "-" + limblabel[ln] + ".pdf")) 
